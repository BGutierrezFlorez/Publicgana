...
export default function Contact() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data?.message || "No fue posible registrar tu interés.");
      }

      setShowSuccess(true);
      setFormData({ nombre: "", correo: "", ciudad: "" });

      setTimeout(() => setShowSuccess(false), 5000);
    } catch (error) {
      const message = error instanceof Error ? error.message : "No fue posible registrar tu interés.";
